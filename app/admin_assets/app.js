...
let tab='tracks',page=1,pages=1,version=0,sort='play_count',direction='desc';
...
async function table(){const current=++version;const p=params();p.set('page',page);p.set('page_size',$('size').value);p.set('q',$('search').value);if(tab==='tracks'){p.set('sort',sort);p.set('direction',direction);for(const [id,key] of [['min-plays','min_plays'],['max-plays','max_plays']]){const input=$(id);if(!input.validity.valid)throw Error('Playback counts must be whole numbers of zero or more.');if(input.value!=='')p.set(key,input.value);}if(p.has('min_plays')&&p.has('max_plays')&&Number(p.get('min_plays'))>Number(p.get('max_plays')))throw Error('Minimum playbacks cannot exceed maximum playbacks.');}const data=await get('/api/admin/repository/'+tab+'?'+p);if(current!==version)return;pages=data.pages;$('table-title').textContent=titles[tab];$('table-note').textContent=data.date_filter_applied?'Filtered by the selected dates.':(tab==='tracks'?'All-time broadcast counts, including music currently playing or interrupted after starting. Range endpoints are inclusive. Click a column heading to sort.':'Complete repository; date selection applies to activity charts and history tables.');$('pagination').textContent=`Page ${page} of ${pages} · ${data.total.toLocaleString()} records`;$('previous').disabled=page<=1;$('next').disabled=page>=pages;const keys=data.items.length?Object.keys(data.items[0]):[];const header=node('tr','');for(const key of keys){const th=node('th','');const label=key==='play_count'?'Playbacks':key.replaceAll('_',' ');if(tab==='tracks'){th.setAttribute('aria-sort',sort===key?(direction==='asc'?'ascending':'descending'):'none');const button=node('button',label+(sort===key?(direction==='asc'?' ↑':' ↓'):''));button.type='button';button.onclick=()=>{direction=sort===key&&direction==='desc'?'asc':'desc';sort=key;page=1;load();};th.append(button);}else th.textContent=label;header.append(th);}$('head').replaceChildren(header);$('rows').replaceChildren(...data.items.map(row=>{const tr=node('tr','');for(const key of keys){let value=row[key];if(value!==null&&['created','accepted','completed','started','finished','downloaded_at','first_seen','last_seen'].includes(key))value=new Date(value*1000).toISOString().replace('T',' ').slice(0,19)+' UTC';const td=node('td',value===null?'—':String(value));tr.append(td);}return tr;}));if(!data.items.length){const tr=node('tr','');tr.append(node('td','No matching records.'));$('rows').append(tr);}}
...
for(const [key,title] of Object.entries(titles)){const b=node('button',title);b.type='button';b.setAttribute('aria-selected',String(key===tab));b.onclick=()=>{tab=key;page=1;$('playback-filters').hidden=tab!=='tracks';for(const e of $('tabs').children)e.setAttribute('aria-selected',String(e===b));load();};$('tabs').append(b);}
$('period').onchange=()=>{if($('period').value!=='custom'){dates();page=1;load(true);}};
$('apply').onclick=()=>{page=1;load(true);};$('refresh').onclick=()=>load(true);$('size').onchange=()=>{page=1;load();};let timer;$('search').oninput=()=>{clearTimeout(timer);timer=setTimeout(()=>{page=1;load();},300);};$('previous').onclick=()=>{if(page>1){page--;load();}};$('next').onclick=()=>{if(page<pages){page++;load();}};for(const id of ['min-plays','max-plays'])$(id).oninput=()=>{clearTimeout(timer);timer=setTimeout(()=>{page=1;load();},300);};$('clear-plays').onclick=()=>{$('min-plays').value='';$('max-plays').value='';page=1;load();};load(true);
